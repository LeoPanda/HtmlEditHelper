...
class IndexIframeHelper extends PanelBase{
	...
	private ListBoxField numOfWidth 
			= new ListBoxField("numOfWidth","１行の最大コマ数:",null,setNumValueList(MAX_WIDTH));
	private TextBoxField iFrameWidth 
			= new TextBoxField("iFrameWidth","iframe横幅:",new ValidateBase[]{isRequired,isNumeric});
	private ListBoxField iFrameHight 
			= new ListBoxField("iFrameHight","iframe高さ:",null,setNumValueList(MAX_IFRAME_HEIGHT));
	/*
	 * コンストラクタ
	 */
	...
	public boolean validateFields(){
		...
		//複合チェック
		if(!isRequired.validate(queryString.getText())
		&& !isRequired.validate(caterogy.getText())){
	 ...
	 */
	 private String getIframeStyle() {
			return  "border: none;"
					+ "width: " + iFrameWidth.getText() 
								+"px;"
					+ "height:" + getIframeHeight(iFrameHight.getText()) 
								+ "px;";
	}
	/*
	 * iframeの高さを計算する
